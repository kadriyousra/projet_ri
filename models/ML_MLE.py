...
class LanguageModelMLE:
    """Language Model avec Maximum Likelihood Estimation (MLE) - SANS lissage"""
    ...
    def calculate_p_mle(self, term: str, doc_id: int) -> float:
        """
        ...
        Returns:
            Probabilité MLE (peut être 0 si le terme n'existe pas dans le doc)
        """
    ...
    def calculate_rsv(self, query_terms: List[str], doc_id: int) -> float:
        """
        Calcule le RSV (Retrieval Status Value) avec MLE sans lissage
        
        RSV(Q, d) = ∏ P_MLE(w|d) pour w ∈ Q
        
        IMPORTANT: Si UN SEUL terme a une probabilité de 0, le RSV = 0
        Comme dans le slide: information n'existe pas → P = 0 → RSV = 0
        ...
        Returns:
            RSV score (produit des probabilités)
        """
        rsv = 1.0
        
        for term in query_terms:
            p_mle = self.calculate_p_mle(term, doc_id)
            
            # Si un terme a une probabilité de 0, le produit = 0
            if p_mle == 0:
                return 0.0
...
if __name__ == "__main__":
    ...
    print("="*80)
    print("ÉVALUATION COMPLÈTE - LANGUAGE MODEL MLE (SANS LISSAGE)")
    print("="*80)
    
    # 1. Créer le modèle
    print("\n📚 Étape 1: Initialisation du modèle Language Model MLE")
    print("   Technique: Maximum Likelihood Estimation (unigram)")
    print("   Lissage: AUCUN (peut produire des scores de 0)")
    lm_mle = LanguageModelMLE(
        inverted_index_path=INVERTED_INDEX_PATH,
        doc_term_matrix_path=DOC_TERM_MATRIX_PATH
    )
    print(f"✅ Modèle initialisé")
    print(f"   - Documents: {lm_mle.N}")
    print(f"   - Vocabulaire: {len(lm_mle.inverted_index)} termes")
    ...
    metrics = IRMetrics(relevance_judgments, model_name="LM_MLE")

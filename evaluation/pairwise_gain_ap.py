"""
Calcul du Gain (%) DEUX À DEUX - VERSION AVERAGE PRECISION
...
def extract_ap_from_file(filepath: str, query_ids: List[int] = None) -> Dict[int, float]:
    """
    Extrait les scores Average Precision d'un fichier de résultats
        ...
        for i in range(1, len(query_blocks), 2):
            if i + 1 < len(query_blocks):
                ...
                if query_id in query_ids:
                    # Chercher Average Precision dans la section MÉTRIQUES
                    ...
                    if metrics_match:
                        ...
                        # Chercher Average Precision (pas AP Interpolée)
...
def load_all_results(results_dir: str = None, query_ids: List[int] = None) -> Dict[str, Dict[int, float]]:
    ...
    print("\n" + "="*150)
    print("📂 CHARGEMENT DES FICHIERS - AVERAGE PRECISION")
    print("="*150)
    print(f"Dossier: {results_dir}")
    print(f"Requêtes: Q{query_ids[0]}-Q{query_ids[-1]} (I{query_ids[0]}-I{query_ids[-1]} du MED.QRY)")
    print(f"Métrique: Average Precision (AP)")
    print("-"*150)
    ...
    for result_file in sorted(result_files):
        ...
        if ap_scores:
            ...
            print(f"      ✅ {num_queries} scores AP extraits (MAP: {mean_ap:.4f})")
            ...
            print(f"      ⚠️  Aucun score AP trouvé")
        
        print()
    
    print("-"*150)
    print(f"✅ Total: {len(all_ap_data)} modèles chargés")
    print("="*150)
...
class PairwiseGainCalculator:
    """Classe pour calculer les gains deux à deux - VERSION AP"""
    ...
    def calculate_gain(self, model_a: str, model_b: str, query_id: int) -> float:
        """Calcule le gain de model_a par rapport à model_b pour une requête"""
    ...
    def print_pairwise_matrix(self, query_ids: List[int] = None):
        ...
        print("\n" + "="*150)
        print("📊 MATRICE DE GAINS DEUX À DEUX - Average Precision")
        print("="*150)
        print(f"Requêtes: Q{query_ids[0]}-Q{query_ids[-1]} (I{query_ids[0]}-I{query_ids[-1]} du LAB 5)")
        print("\n💡 Lecture:")
    ...
    def print_comparison(self, model_a: str, model_b: str, query_ids: List[int] = None):
        ...
        print("\n" + "="*150)
        print(f"⚖️  COMPARAISON: {model_a} vs {model_b} (Average Precision)")
        print("="*150)
        ...
        # Seuil de 5% pour déterminer si la différence est significative
        SEUIL = 5.0
    ...
    def print_top_differences(self, query_ids: List[int] = None, top_n: int = 10):
        ...
        print("\n" + "="*150)
        print(f"🔝 TOP {top_n} DIFFÉRENCES (Average Precision)")
        print("="*150)
    ...
    def save_all_results(self, output_dir: str = "results/gainAp", query_ids: List[int] = None):
        ...
        print(f"✅ Matrice AP: {matrix_file}")
        ...
        print(f"✅ Comparaisons AP: {comparisons_dir}/")
        
        # Rapport
        report_file = os.path.join(output_dir, "pairwise_gain_report_AP.txt")
        with open(report_file, 'w', encoding='utf-8') as f:
            f.write("="*150 + "\n")
            f.write("ANALYSE DES GAINS DEUX À DEUX - Average Precision\n")
            f.write("="*150 + "\n\n")
            f.write("MATRICE:\n")
            f.write("-"*150 + "\n")
        ...
        print(f"✅ Rapport AP: {report_file}")


# ==================== FONCTIONS PRINCIPALES ====================

def pairwise_analysis_from_files(results_dir: str = None, query_ids: List[int] = None):
    """Analyse complète avec Average Precision"""
    if query_ids is None:
        query_ids = list(range(1, 11))
    
    print("\n" + "="*150)
    print("🔄 ANALYSE DEUX À DEUX - AVERAGE PRECISION (I1-I10)")
    print("="*150)
    ...
    calc.save_all_results(output_dir="results/gainAp", query_ids=query_ids)
    
    return calc


def compare_two_models(model_a: str, model_b: str, results_dir: str = None, query_ids: List[int] = None):
    """Compare deux modèles avec Average Precision"""
    if query_ids is None:
        query_ids = list(range(1, 11))

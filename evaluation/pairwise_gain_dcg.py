...
def load_all_results(results_dir: str = None, query_ids: List[int] = None) -> Dict[str, Dict[int, float]]:
    ...
    print("\n" + "="*150)
    print("📂 CHARGEMENT DES FICHIERS - UTILISATION DE DCG@20")
    print("="*150)
    print(f"Dossier: {results_dir}")
    print(f"Requêtes: Q{query_ids[0]}-Q{query_ids[-1]}")
    print(f"Métrique: DCG@20 (non normalisé)")
    print("-"*150)
...
class PairwiseGainCalculator:
    """Classe pour calculer les gains deux à deux - VERSION DCG@20"""
    ...
    def calculate_gain(self, model_a: str, model_b: str, query_id: int) -> float:
        ...
        if dcg_b == 0.0:
            return 0.0 if dcg_a == 0.0 else float('inf')
        
        return ((dcg_a - dcg_b) / dcg_b) * 100
    ...
    def print_pairwise_matrix(self, query_ids: List[int] = None):
        ...
        print("\n" + "="*150)
        print("📊 MATRICE DE GAINS DEUX À DEUX - Basée sur DCG@20")
        print("="*150)
        print(f"Requêtes: Q{query_ids[0]}-Q{query_ids[-1]}")
        print("\n💡 Lecture:")
    ...
    def print_comparison(self, model_a: str, model_b: str, query_ids: List[int] = None):
        ...
        print("\n" + "="*150)
        print(f"⚖️  COMPARAISON: {model_a} vs {model_b} (DCG@20)")
        print("="*150)
    ...
    def save_all_results(self, output_dir: str = "results", query_ids: List[int] = None):
        ...
        # Matrice
        matrix_df = self.generate_pairwise_matrix(query_ids)
        matrix_file = os.path.join(output_dir, "pairwise_gain_matrix_DCG.csv")
        matrix_df.to_csv(matrix_file)
        print(f"✅ Matrice DCG: {matrix_file}")
        
        # Comparaisons
        comparisons_dir = os.path.join(output_dir, "pairwise_comparisons_DCG")
        ...
        # Rapport
        report_file = os.path.join(output_dir, "pairwise_gain_report_DCG.txt")
        with open(report_file, 'w', encoding='utf-8') as f:
            f.write("="*150 + "\n")
            f.write("ANALYSE DES GAINS DEUX À DEUX - DCG@20\n")
            f.write("="*150 + "\n\n")
            f.write("MATRICE:\n")
            f.write("-"*150 + "\n")
        ...
        print(f"✅ Rapport DCG: {report_file}")


# ==================== FONCTIONS PRINCIPALES ====================

def pairwise_analysis_from_files(results_dir: str = None, query_ids: List[int] = None):
    """Analyse complète avec DCG@20"""
    if query_ids is None:
        query_ids = list(range(1, 11))
    
    print("\n" + "="*150)
    print("🔄 ANALYSE DEUX À DEUX - DCG@20")
    print("="*150)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if len(sys.argv) >= 3:
            model_a = sys.argv[1]
            model_b = sys.argv[2]
            print(f"\n🔍 Comparaison DCG@20: {model_a} vs {model_b}")
